...
void tryConnect() {
  ...
  while (!Serial) { //wait for connection
    connected = false;
    if (millis() - timer > 100ul) { //led blink - if not connected
      timer = millis();
      digitalWrite(LED_BUILTIN, lighting);
      lighting = !lighting; 
    }
  }
  ...
  if (!connected) {
    if (Serial) { // connected now
      connected = true;
      digitalWrite(LED, HIGH);
      delay(100);
      Serial.println(F("#FOTOPLETYSMOGRAM V1#"));
    }
  }
}
